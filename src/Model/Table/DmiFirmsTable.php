...
class DmiFirmsTable extends Table{
	...
	public function checkApplicantExportUnit($customer_id) {
		...
		////echo"<pre>";print_r($check_application_type);exit;
	...
	public function newlyAddedFirmListReportConditionsForStats($application_type,$company_id,$state,$district,$search_from_date,$search_to_date,$search_flag,$cert_type) {		
		...
		{			
			...
			else 
			{
				...
				$firms_details = $this->find('all')->select(['id', 'customer_primary_id', 'created', 'customer_id','certification_type'])->where(['certification_type'=>$cert_type_decode])->group(['id', 'customer_primary_id'])->order(['created'=>'DESC'])->toArray();
			}
		}
