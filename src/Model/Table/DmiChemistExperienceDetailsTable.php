	...
	public function saveFormDetails($chemist_id,$forms_data) {
		...
		if($dataValidatation == 1 ){
			...
			for ($i=0;$i<$row_count;$i++) {
				...
				//to split the total experince into month and year to save in separate integer fields in database - Akash [16-05-2023]
				...
				if (preg_match($pattern, $total, $matches)) {
					$exp_in_year = (int)$matches[1]; // Extracted years
					$exp_in_month = (int)$matches[2]; // Extracted months
					$exp_in_days = ($exp_in_year * 365) + ($exp_in_month * 30); // Assuming 1 year = 365 days, 1 month = 30 days

					if (isset($matches[4])) {
						$exp_in_days += (int)$matches[4]; // Add extracted days
					}
				} elseif (preg_match('/(\d+)\s+Months?(\s+(\d+)\s+D)?/i', $total, $matches)) {
					$exp_in_year = 0; // No years specified
					$exp_in_month = (int)$matches[1]; // Extracted months
					$exp_in_days = $exp_in_month * 30; // Assuming 1 month = 30 days

					if (isset($matches[3])) {
						$exp_in_days += (int)$matches[3]; // Add extracted days
					}
				} elseif (preg_match('/(\d+)\s+D/i', $total, $matches)) {
					$exp_in_year = 0; // No years specified
					$exp_in_month = 0; // No months specified
					$exp_in_days = (int)$matches[1]; // Extracted days
				} else {
					$exp_in_year = null;
					$exp_in_month = null;
					$exp_in_days = null;
