...
class CustomfunctionsComponent extends Component {
	...
	public function returnGrantDateCondition($customer_id,$application_type=null) {//new argument added on 03-07-2023 "$application_type"
		...
		}elseif ($application_type == 5) { //added on 07-03-2023 by Amol to get FDC

			$DmiGrantCertificatesPdfs = TableRegistry::getTableLocator()->get('Dmi15DigitGrantCertificatePdfs');
			$grantDate = $DmiGrantCertificatesPdfs->find('all',array('conditions'=>array('customer_id IS'=>$customer_id),'order'=>'id DESC'))->first();
		
		}elseif ($application_type == 6) { //added on 07-03-2023 by Amol to get FDC
	...
	public function changeDateFormatNew($date) {
		...
		if (!empty($date)) {
			...
			if (count($result) == 2) {
				$date1 = $result[0];
				$time = $result[1];
				$dateTime = \DateTime::createFromFormat("Y-m-d", $date1);
	
				if ($dateTime instanceof \DateTime) {
					$formattedDate = $dateTime->format("Y-m-d") . ' ' . $time;
				} else {
