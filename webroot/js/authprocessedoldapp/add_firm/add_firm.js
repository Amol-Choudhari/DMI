//// ADD FIRM JS FOR AUTH OLD PROECCESSED APPLICATION	
$( document ).ready(function() {
	
	$('.onchangegetcommodity').change(function (e) { 
		e.preventDefault();
		get_commodity();
	});

	$('.onchangegetdistrict').change(function (e) { 
		e.preventDefault();
		get_district();
	});

	$('.onclickValidation').click(function (e) { 
	...
	$("#total_charge").prop("readonly", true);
	...
	$('#commodity').change(function (e) {
		...
		if($('#commodity option:selected').val()!=''){
			...
			//check if already selected in the list added on 15-09-2021 by Amol
	...
	$('#certification_type').change(function () {
		...
		/*$('#is_already_granted-no').is(":checked");
		$("#is_already_granted-no").prop("checked", true);
	...
	$('#is_already_granted-no').click(function(){
		$("#old_granted_certificate").hide();
		$("#last_renewal_details").hide();
		$("#total_charge_box").show();//added on 28-11-2017 by Amol to show if new application
	});
	...
	$('#primary_id').change(function () {
		...
		var form_data = '';
		form_data = $("#add_firm_form").serializeArray();
		form_data.push(	{name: "primary_id",value: primary_id});

		$.ajax({
			type: "POST",
			url: "../authprocessedoldapp/primary_user_detail",
			data: form_data,
			beforeSend: function (xhr) { // Add this line
				xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());
			},
			success: function(response){
				$("#show_firm_name").html(response);
			}
		});
	});
